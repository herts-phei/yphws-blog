...
format: html
editor: visual
image: "HCC 2023 - Priory School - high-res-47.jpg"
---
...
# yphws_lookup <- pin_read(board, "ayu/yphws_lookup")
...
tables_df_suppressed_proportions <- tables_df_suppressed %>% map(~ .x %>% as.matrix %>% prop.table(margin = 1) %>% `*`(100) %>% round(0) %>% as.data.frame() %>% rownames_to_column("care-experienced") %>% mutate(across(!"care-experienced", ~ paste0(.x, "%"))))
...
```{r}
#| label: fig-stack-bar-graph
#| fig-cap: "Normalised bar graph showing proportion of answers for care-experienced students and non care-experienced students on continuing education after leaving school"
#| echo: false
#| message: false
 
tables_df_suppressed[["education"]] %>% pivot_longer(
  cols = c("Maybe", "No", "Yes"),
  names_to = "answer",
  values_to = "number"
) %>% 
  dplyr::mutate(dplyr::across(`care-experienced`, ~ case_when(. == "No" ~ "Non care-experienced\nstudents", 
                                                            . == "Yes" ~ "Care-experienced\nstudents",
                                                            TRUE ~ NA)
                              )
                ) %>% 
  ggplot2::ggplot(aes(`care-experienced`, number, fill = answer, group = answer)) + 
...
The plot above shows differences in the relative proportions of care-experienced and non care-experienced students' responses to pursuing education. A difference can be observed in the category of the responses through a visual glance. After performing a chi-squared test analysis, this difference is found to be statistically significant.
...
The below shows tables for the non-significant pathways.
